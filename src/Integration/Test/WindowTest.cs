...
public class WindowTest : IntegrationTestBase {
    ...
    public async Task WebViewFlowIsCorrect() {
        IFolder logFolder = new Folder(Path.GetTempPath()).SubFolder("AspenlaubLogs").SubFolder("VishizhukelNetWeb");
        DateTime minChangedAt = DateTime.Now;
        await LaunchWindowAndGoToLocalhost(logFolder, minChangedAt);
        List<ISimpleLogEntry> allActualLogEntries = GetLogEntriesInFilesChangedAfter(logFolder, minChangedAt);
        ...
        Assert.IsGreaterThan(expectedLines.Count, actualLines.Count, $"Expected {expectedLines.Count} log lines, got {actualLines.Count}");
        int expectedLineIndex = 0, actualLineIndex = 0, optionalsFound = 0;
        string actualLine;
        while (expectedLineIndex < expectedLines.Count) {
            string expectedLine = expectedLines[expectedLineIndex];
            ...
            if (expectedLine.StartsWith('{')) {
                ...
                optionalsFound ++;
                Assert.IsLessThan(2, optionalsFound, "More than two optional lines found, this is unexpected");
            }
        ...
        double elapsedSeconds = (actualLogEntries[^1].LogTime - actualLogEntries[0].LogTime).TotalSeconds;
        Assert.IsLessThan(maxSeconds, elapsedSeconds, $"Expected navigation to take less than {maxSeconds} seconds, it was {elapsedSeconds}");
    }
    ...
    private List<string> GetExpectedLines() {
        string folderName = GetType().Assembly.Location;
        folderName = folderName.Substring(0, folderName.LastIndexOf('\\'));
        string masterLogFileName = new Folder(folderName).FullName + @"\ExpectedWebViewFlowLog.txt";
